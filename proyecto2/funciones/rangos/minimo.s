...
.data
    ...
    mensajeMinimo:
        .asciz " || Operacion MINIMO realizada con exito\n"
        lenMensajeMinimo = .- mensajeMinimo
    
    errorMinimo:
        .asciz " || Error en el MINIMO\n"
        lenErrorMinimo = .- errorMinimo
...
Proc_Minimo:
    ...
    minimoLoop:
        ...
        CBZ w2, minimo_first_cell      // Si w2 es 0, saltar a guardarSumaVal

        CMP w2, w3              // Comparamos w2 con w3
        BNE minimo_error          // Si no son iguales, saltar a errorSuma

        B minimoLoop          // Si no se cumple la condicion, repetir el ciclo
    ...
    minimo_first_cell:
        ...
        minimo_first_Loop:
            ...
            BEQ minimo_continue   // Si es un espacio se continua con el resto del comando

            CMP w2, 65                  // Se compara si el byte es una A
            BGE minimo_first_column       // Si es mayor o igual a A se asume que es una columna

            CMP w2, 48                  // Se compara si el byte es un 0
            BGE minimo_first_row          // Si es mayor o igual a 0 se asume que es una fila

            CMP w2, 45                  // Se compara si el byte es un -
            BEQ minimo_error              // Si es un - se asume que es un error

            B minimo_error                // Si no es ninguno de los anteriores se asume que es un error

            minimo_first_column:

                CMP w2, 75              // Se compara si el byte es una K
                BGT minimo_error          // Si es mayor a K se asume que es un error
            ...
            minimo_first_row:
                ...
                BGT minimo_error          // Si es mayor a 9 se asume que es un error
    ...
    minimo_continue:
        ...
        CBZ x23, minimo_error         // Si no se encontro la columna se asume que es un error
        CBZ x24, minimo_error         // Si no se encontro la fila se asume que es un error
        ...
        minimo_cont_Loop:
            ...
            CBZ w2, minimo_second_cell  // Si se llega al final del comando HASTA se continua con la siguiente celda

            CMP w2, w3              // Se compara si los bytes son iguales
            BNE minimo_error          // Si no son iguales se asume que es un error
    ...
    minimo_second_cell:
        ...
        minimo_second_Loop:
            ...
            CMP w2, 32          // Se compara si el byte es un espacio
            BEQ Minimo_Ejecutar   // Si es un espacio se continua con el resto del comando

            CMP w2, 10          // Se compara si el byte es un salto de linea
            BEQ Minimo_Ejecutar   // Si es un salto de linea se continua con el resto del comando

            CMP w2, 65                  // Se compara si el byte es una A
            BGE minimo_second_column      // Si es mayor o igual a A se asume que es una columna

            CMP w2, 48                  // Se compara si el byte es un 0
            BGE minimo_second_row         // Si es mayor o igual a 0 se asume que es una fila
    ...
    Minimo_Ejecutar:
        ...
        BNE compare_row_min         // Si no son iguales se asume que es en la misma fila
        ...
        compare_row_min:
            ...
            BNE minimo_error              // Si no son iguales, entonces es asume que es un error

            B same_row_min
        
        // ---------------------------- SE ANALIZA LA MISMA COLUMNA | EJEMPLO = MINIMO DESDE A1 HASTA A16 ----------------------------
        same_column_min:
            ...
            BGT minimo_error              // Si la fila de inicio es mayor a la fila de fin se asume que es un error

            MOV x12, #23                // Se carga el valor de la columna en x12

            CMP w9, w12                 // Se compara si la fila de fin es mayor a 23
            BGT minimo_error              // Si la fila de fin es mayor a 23 se asume que es un error
            ...
            same_column_pre_min:
                ...
                MIN_same_column_Loop:
                    ...
                    CBNZ w18, MIN_column_non_first_value  // Si no es la primera vez que entra al ciclo, que continue

                    MOV x18, 1              // Se cambia el flag de primera vez a 1

                    STR x9, [x6]           // Se guarda el valor en la variable retorno

                    B MIN_col_continue_Loop

                    MIN_column_non_first_value:
                        
                        CMP w9, w5             // Se compara si el valor de retorno, es mayor al valor de la arreglo
                        BGE MIN_col_continue_Loop      // Si es mayor se continua con el loop

                        STR x9, [x6]           // Se guarda el valor en la variable retorno

                    MIN_col_continue_Loop:

                        CMP x21, x7             // Se compara si la fila de inicio es igual a la fila de fin
                        BEQ MIN_same_column_end     // Si son iguales se termina el llenado

                        ADD x21, x21, 1         // Se suma 1 a la fila de inicio

                    B MIN_same_column_Loop
        ...
        // ---------------------------- SE ANALIZA LA MISMA FILA | EJEMPLO = MINIMO DESDE A1 HASTA G1 ----------------------------
        same_row_min:
            ...
            BGT minimo_error          // Si la columna 1 es mayor a la columna 2 se asume que es un error
            ...
            MIN_same_row_pre:
                ...
                MIN_same_row_Loop:
                    ...
                    CBNZ w18, MIN_row_non_first_value  // Si no es la primera vez que entra al ciclo, que continue

                    MOV x18, 1              // Se cambia el flag de primera vez a 1

                    STR x9, [x6]           // Se guarda el valor en la variable retorno

                    B MIN_row_continue_Loop

                    MIN_row_non_first_value:

                        CMP w9, w5             // Se compara si el valor de retorno, es mayor al valor de la arreglo
                        BGE MIN_row_continue_Loop      // Si es mayor se continua con el loop

                        STR x9, [x6]           // Se guarda el valor en la variable retorno
                    
                    MIN_row_continue_Loop:

                        CMP x21, x7                 // Se compara si la columna de inicio es igual a la columna de fin
                        BEQ MIN_same_row_end          // Si son iguales se termina el llenado
    ...
    minimo_error:
        ...
        print errorMinimo, lenErrorMinimo
